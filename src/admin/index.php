...
<body class="bg-dark text-gray-200 flex items-center justify-center min-h-screen">
    <div class="flex flex-col justify-center  mx-auto p-6 bg-light-10 rounded-lg shadow-md w-full max-w-4xl">
        ...
        <div class="flex justify-center">
            <a href="../logout.php" class=" text-center text-white p-2 rounded-lg bg-red-500 hover:bg-red-700 mb-4">Logout</a>
        </div>
        ...
        <table class="min-w-full bg-gray-100 border border-gray-300 rounded-lg">
            <thead>
                <tr>
                    ...
                    <th class="py-2 px-4 text-left text-dark border-b border-gray-300 bg-gray-200">Registrants</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($events as $event): ?>
                    <tr class="hover:bg-gray-200">
                        ...
                        <td class="py-2 px-4 text-dark border-b border-gray-300">
                            <?= htmlspecialchars($event['location']); ?></td>
                        <td class="py-2 px-4 text-dark border-b border-gray-300">
                            <?php if ($event['image']): ?>
                                <img src="../uploads/<?= htmlspecialchars($event['image']); ?>"
                                    alt="<?= htmlspecialchars($event['name']); ?>" class="w-24 h-auto">
                            <?php else: ?>
                                No Image
                            <?php endif; ?>
                        </td>
                        <td class="py-2 text-dark px-4 border-b border-gray-300">
                            <?php
        ...
        <div class="mt-6 flex justify-between">
            <a href="manage_events.php"
                class="bg-gradient text-white py-2 px-4 rounded-lg  transition duration-300">Manage Events</a>
            <a href="manage_users.php"
                class="bg-gradient text-white py-2 px-4 rounded-lg  transition duration-300">Manage Users</a>
        </div>
    </div>
</body>
